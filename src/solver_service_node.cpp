...
{
    auto logger = rclcpp::get_logger("temp");
    RCLCPP_INFO(logger, "callback");
    // Process the request (resize and add data)
    for (size_t i = 0; i < sizeof(forces_params_.x0); i++)
        forces_params_.x0[i] = request->x0.data[i];
    // forces_params_.x0 = request->x0.data;

    for (size_t i = 0; i < sizeof(forces_params_.all_parameters); i++)
        forces_params_.all_parameters[i] = request->x0.data[i];

    for (size_t i = 0; i < sizeof(forces_params_.xinit); i++)
        forces_params_.xinit[i] = request->xinit.data[i];
    RCLCPP_INFO(logger, "solve");

    response->exit_code = Solve(); // Solve the optimization problem
    RCLCPP_INFO(logger, "done solve");

    // Fill the response
    /** @todo This should probably be autogenerated depending on what your horizon length is*/
    size_t j = 0;
    size_t num_var = sizeof(forces_output_.x01);
